    ...
    for (; iter != end; ++iter) {
        ...
        for (; siter != send; ++siter) {
            ...
            if (verbose) {
                ...
                if (name.length()>widthField) {
                    name.replace(widthField-1,1,'*');
                }
                os.output().width(widthField); os << name.at(0,widthField);
    ...
    if (nfields <= 0) {
        ...
        for (; fiter!=fend; ++fiter) {
            ...
            if (fld >=0 && fld < (Int)nfields) {
                ...
                if (name.length()>widthName) {
                    name.replace(widthName-1,1,"*");
                }
                os.output().setf(ios::left, ios::adjustfield);
                os.output().width(widthLead);    os << "  ";
...
// end of included & modified MSSummary.cc
