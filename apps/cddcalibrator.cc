/// @file cddcalibrator.cc
///
/// @brief Perform calibration, including direction-dependent solutions,
/// and writes the results in a parset file or casa table.
...
ASKAP_LOGGER(logger, ".cddcalibrator");

using namespace std;
using namespace askap;
using namespace askap::synthesis;

class CDDcalibratorApp : public askap::Application
{
    public:
        virtual int run(int argc, char* argv[])
        {
            ...
            try {
                StatReporter stats;
                LOFAR::ParameterSet subset(config().makeSubset("Cddcalibrator."));
                ...
                DDCalibratorParallel calib(comms, subset);
