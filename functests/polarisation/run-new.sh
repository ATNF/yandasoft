...
tar -xvf ../vela-new.tar
echo Done

mpirun -np 2 $IMAGER -c polim-new.in | tee $OUTPUT
if [ $? -ne 0 ]; then
...
BadI=`echo ${PeakI} | awk '{ print $1 - 3.05 }' | sed 's/-//' | awk '{ print ($1 > 0.01) }'`
BadQ=`echo ${PeakQ} | awk '{ print $1 - 0.56 }' | sed 's/-//' | awk '{ print ($1 > 0.01) }'`
BadU=`echo ${PeakU} | awk '{ print $1 - 2.10 }' | sed 's/-//' | awk '{ print ($1 > 0.01) }'`
