...
void AdviseDI::addMissingParameters(bool extra)
{
   ...
   if (missing) {
       ...
       if (itsParset.getBool("Images.nyquistgridding",false)) {
           maxU = advice.maxU();
           maxV = advice.maxV();
           maxW = advice.maxW();
       }
   ...
   ASKAPCHECK(!itsParset.isDefined("Images.extraoversampling"), "Images.extraoversampling cannot be set by user");
   if (itsParset.getBool("Images.nyquistgridding",false) || itsParset.isDefined("Images.griddingcellsize")) {

       ASKAPCHECK(itsParset.isDefined("Images.cellsize") && itsParset.isDefined("Images.shape"),
           "The global image cellsize and shape are currently required with Nyquist gridding");

       const std::vector<double>
           cellSize = SynthesisParamsHelper::convertQuantity(itsParset.getStringVector("Images.cellsize"),"arcsec");
       const std::vector<int> imSize = itsParset.getInt32Vector("Images.shape");
       ...
       if (itsParset.isDefined("Images.griddingcellsize")) {
           const std::vector<string> gParam = itsParset.getStringVector("Images.griddingcellsize");
       ...
       ASKAPLOG_INFO_STR(logger, "  Changing cellsize from "<<itsParset.getStringVector("Images.cellsize")<<
                                 " to "<<"["<<gCellSize[0]<<"arcsec,"<<gCellSize[1]<<"arcsec]");
       ASKAPLOG_INFO_STR(logger, "  Changing shape from "<<itsParset.getInt32Vector("Images.shape")<<
       ...
       if (extraOsFactor > 1.) {
           itsParset.add("Images.extraoversampling", utility::toString(extraOsFactor));
           itsParset.replace("Images.cellsize", "["+toString(gCellSize[0])+"arcsec,"+toString(gCellSize[1])+"arcsec]");
           itsParset.replace("Images.shape", "["+toString(long(nPix))+","+toString(long(nPix))+"]");
           if (itsParset.isDefined("Images.subshape")) {
             vector<int> subshape = itsParset.getInt32Vector("Images.subshape");
             int subPix = subshape[0] / extraOsFactor;
             subPix += subPix % 2;
             itsParset.replace("Images.subshape","["+toString(long(subPix))+","+toString(long(subPix))+"]");
             ASKAPLOG_INFO_STR(logger, "  Changing subshape from "<< subshape <<
                                       " to "<<"["<<long(subPix)<<","<<long(subPix)<<"]");
           }
       }
   }
