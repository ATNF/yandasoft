...
#include <askap/parallel/DDCalibratorParallel.h>
      ...
      itsMatrixIsParallel(false), itsMajorLoopIterationNumber(0)
{
  ...
  if (what2solve.find("bandpass") != std::string::npos) {
      ...
      ASKAPCHECK(!itsSolveGains && !itsSolveLeakage,
         "Combination of frequency-dependent and frequency-independent effects is not"
  ...
  if (itsComms.isMaster()) {
      ...
      if (calAccType == "service") {
        ...
        boost::shared_ptr<ServiceCalSolutionSource>
            src = boost::dynamic_pointer_cast<ServiceCalSolutionSource>(itsSolutionSource);
  ...
  if (itsComms.isWorker()) {
      ...
      if (itsChannelsPerWorker > 0) {
          ...
          ASKAPLOG_INFO_STR(logger, "This worker at rank = "<<itsComms.rank()<<" will process "<<
              itsChannelsPerWorker<<" spectral channels starting from "<<itsStartChan<<" (chunk="<<chunk<<")");
      }
      ...
      if (itsSolutionInterval < 0) {
          ...
          ASKAPLOG_INFO_STR(logger, "Solution will be made for each "<<itsSolutionInterval<<
              " seconds chunk of the dataset");
      }
  }
}
...
/// is sufficient.
/// @param[in] parset ParameterSet for inputs
void DDCalibratorParallel::init(const LOFAR::ParameterSet& parset)
{
  if (itsComms.isMaster()) {
      ...
      if (itsSolveGains) {
          ...
          for (casacore::uInt ant = 0; ant<nAnt; ++ant) {
               for (casacore::uInt cal = 0; cal<nCal; ++cal) {
                    ...
                    if ((ant == 0) || (ant == 3)) {
                         itsModel->fix(accessors::CalParamNameHelper::paramName(ant, cal, casacore::Stokes::XX));
                         itsModel->fix(accessors::CalParamNameHelper::paramName(ant, cal, casacore::Stokes::YY));
                    }
                    //
                    */
               }
      ...
      if (itsSolveLeakage) {
          ...
          for (casa::uInt ant = 0; ant<nAnt; ++ant) {
               for (casa::uInt cal = 0; cal < nCal; ++cal) {
                    itsModel->add(accessors::CalParamNameHelper::paramName(ant, cal, casa::Stokes::XY),
                                  casa::Complex(0.,0.));
                    itsModel->add(accessors::CalParamNameHelper::paramName(ant, cal, casa::Stokes::YX),
                                  casa::Complex(0.,0.));
               }
          }
          // technically could've done it outside the if-statement but this way it reflects the intention to keep track
      ...
      if (itsSolveBandpass) {
          ...
          ASKAPLOG_INFO_STR(logger, "Initialise bandpass (unknowns) for "<<nAnt<<" antennas, "<<nCal<<" cal(s) and "<<
                                   nChan<<" spectral channels");
          for (casacore::uInt ant = 0; ant<nAnt; ++ant) {
               for (casacore::uInt cal = 0; cal<nCal; ++cal) {
                    const std::string xxParName = accessors::CalParamNameHelper::bpPrefix() +
                        accessors::CalParamNameHelper::paramName(ant, cal, casacore::Stokes::XX);
                    const std::string yyParName = accessors::CalParamNameHelper::bpPrefix() +
                        accessors::CalParamNameHelper::paramName(ant, cal, casacore::Stokes::YY);
                    for (casacore::uInt chan = 0; chan<nChan; ++chan) {
                         itsModel->add(accessors::CalParamNameHelper::addChannelInfo(xxParName, chan),
                                  casacore::Complex(1.,0.));
                         itsModel->add(accessors::CalParamNameHelper::addChannelInfo(yyParName, chan),
                                  casacore::Complex(1.,0.));
                    }
               }
          }
...
void DDCalibratorParallel::calcOne(const std::string& ms, bool discard)
{
  ...
  if ((!itsEquation) || discard) {
      ...
      if (!itsIteratorAdapter) {
          ...
          if (itsChannelsPerWorker > 0) {
              ASKAPLOG_INFO_STR(logger, "Setting up selector for "<<itsChannelsPerWorker<<
                  " channels starting from "<<itsStartChan);
...
bool DDCalibratorParallel::useLinearSolver() const {
...
void DDCalibratorParallel::createCalibrationME(const IDataSharedIter &dsi,
                const boost::shared_ptr<IMeasurementEquation const> &perfectME)
{
  ...
  if (!doPreAveraging)  {
   ...
   ASKAPDEBUGASSERT(itsMaxNCalForPreAvg > 0);
   ASKAPDEBUGASSERT(itsMaxNChanForPreAvg > 0);
   // DAM could add a similar check for itsMaxNCalForPreAvg and DD
   ...
   for (std::vector<std::string>::const_iterator ci=params.begin(); ci != params.end(); ++ci) {
        ...
        if (pol < stokes.nelements()) {
            // for other than bandpass calibration case it is ok to pass 0 as the channel number
            //  - this is how we solve for gains/leakages
            // this is DD cal, and it is assumed that all "beams" use the same data
            //if (preAvgME->hasDataAccumulated(parsed.first.antenna(), parsed.first.beam(), pol, curChan)) {
            if (preAvgME->hasDataAccumulated(parsed.first.antenna(), 0, pol, curChan)) {
                continue;
            }
        }
...
void DDCalibratorParallel::setChannelOffsetInModel() const {
...
void DDCalibratorParallel::calcNE()
{
...
void DDCalibratorParallel::solveNE()
{
  ASKAPLOG_INFO_STR(logger, "Started DDCalibratorParallel::solveNE()");
...
void DDCalibratorParallel::doPhaseReferencing()
{
    if (itsComms.isMaster()) {
        if (itsRefGainXX != "") {
...
void DDCalibratorParallel::rotatePhases()
{
  ...
  if (itsSolveBandpass) {
      ...
      for (casa::uInt chan = 0; chan <= maxChan; ++chan) {
           const std::string xRefPar = accessors::CalParamNameHelper::bpPrefix() +
                                       accessors::CalParamNameHelper::addChannelInfo(itsRefGainXX, chan);
           const std::string yRefPar = accessors::CalParamNameHelper::bpPrefix() +
                                       accessors::CalParamNameHelper::addChannelInfo(itsRefGainYY, chan);
...
double DDCalibratorParallel::solutionTime() const
{
...
void DDCalibratorParallel::setNextChunkFlag(const bool flag)
{
...
bool DDCalibratorParallel::nextChunk() const
{
...
void DDCalibratorParallel::writeModel(const std::string &postfix)
{
  if (itsComms.isMaster()) {
...
void DDCalibratorParallel::sendModelToMaster(const scimath::Params &model) const {
...
void DDCalibratorParallel::receiveModelOnMaster(scimath::Params &model, int rank) {
    ASKAPDEBUGASSERT(itsComms.isMaster());
    ASKAPDEBUGASSERT(itsComms.rank() == 0);
