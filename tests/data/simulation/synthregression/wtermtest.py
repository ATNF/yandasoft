...
def analyseResult(spr, checkWeights=True):
   ...
   '''
    ...
    if disterr > 8:
        raise RuntimeError(
            "Offset between true and expected position exceeds 1 cell size (8 arcsec), d=%f, true_peak=%s" % (
            disterr, true_peak))
    ...
    if abs(stats['peak'] - 1.) > 0.01:
        raise RuntimeError(
            "Peak flux in the preconditioned psf image is notably different from 1.0, F=%f" % stats['peak'])

    if checkWeights:
        stats = spr.imageStats('weights.field1')
        print("Statistics for weight image: ", stats)
        if abs(stats['rms'] - stats['peak']) > 0.1 or abs(stats['rms'] - stats['median']) > 0.1 or abs(
                stats['peak'] - stats['median']) > 0.1:
