...
static void mergeMPI(const LOFAR::ParameterSet &parset, askap::askapparallel::AskapParallel &comms) {
  ...
  for(map<string,string>::iterator ii=outWgtNames.begin(); ii!=outWgtNames.end(); ++ii) {
    ...
    // we now have a vector of shapes. I now want to iterate over the
    // channel - so the accumulator is actually only one channel wide
    ...
    if (comms.isMaster()) { // build this cube - does not need to loop over the outWgtNames
    ...
    // lets do this one plane at a time
    casa::IPosition sliceShape = accumulator.outShape();
    sliceShape[3] = 1; // single sliceShape
    ...
    for (; planeIter.hasMore(); planeIter.next()) {
      ...
      for (uInt img = 0; img < inImgNames.size(); ++img ) {
        ...
        casa::IPosition out_curpos = curpos;
